...
killen de app niet meer, autosave bewaart werk bij power-cuts, en
een Showtime-lock blokkeert destructieve edits zodra het publiek
binnen is.

### Toegevoegd
- **Crash-handler** (`livefire/crash.py`). Installeert `sys.excepthook`,
  `threading.excepthook` en `qInstallMessageHandler` zodat onverwachte
  Python- of Qt-exceptions worden gelogd naar
  ``%APPDATA%\liveFire\logs\crash-YYYYMMDD-HHMMSS.log`` en de operator
  een non-modale dialog ziet ("show draait door, save je workspace,
  log staat hier"). KeyboardInterrupt blijft door de Python-default
  gaan zodat Ctrl+C in dev-mode werkt. Crash binnen de dialog-callback
  zelf wordt netjes afgevangen — de crash-handler genereert nooit een
  tweede crash.
- **Autosave** (`livefire/autosave.py`). `AutosaveManager` schrijft
  iedere 30 s + na elke succesvolle GO een sidecar-bestand naast de
  workspace (``foo.livefire.autosave``). Onbenoemde workspaces landen
  in ``%APPDATA%\liveFire\autosave\untitled-<sessionid>.livefire.autosave``.
  Atomic write via temp-file + ``os.replace`` voorkomt half-geschreven
  JSON tijdens een power-cut. Skipt schrijven wanneer de workspace
  niet dirty is en er al een sidecar staat (bespaart SSD-leven op
  show-PCs).
- **Recovery-prompts**. Bij ``Open …``: detect een autosave die
  nieuwer is dan de workspace en bied 'm aan ter herstel. Bij startup:
  scan de app-autosave-folder op orphan untitled-bestanden uit
  gecrashte sessies; bied de meest recente aan (alleen als de huidige
  workspace nog leeg is, anders zou een onbedachte Yes data weggooien).
- **Showtime-lock** in de transport-bar (🔓/🔒-toggle). Blokkeert alle
  destructive paden: Cut/Copy/Paste/Delete, drag-reorder, inspector-
  edits, Undo/Redo en alle ``push_*``-mutaties via een centrale
  ``_destructive_blocked``-gate. GO en Stop All blijven altijd live.
  Status-bar toont expliciet "Action blocked — Showtime lock is ON"
  bij een geweerde poging zodat de operator weet waarom er niets
  gebeurt.

### Tests
- 22 nieuwe tests: 11 voor autosave (path-resolutie, atomic-write,
  dirty-elision, recovery-detectie, cleanup), 7 voor crash (install/
  uninstall, log-write, KeyboardInterrupt-passthrough, dialog-callback,
  callback-failure-isolation, threading-excepthook), 4 voor het
  Showtime-toggle-widget. Totaal 164 passed.
