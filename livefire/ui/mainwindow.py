...
from ..autosave import (
    AutosaveManager, RecoverableAutosave, find_orphan_untitled, find_recoverable_for,
)
...
class MainWindow(QMainWindow):
    # Cross-thread crash-notificatie. Wordt door crash.py vanuit een
    # niet-Qt-thread geëmit; de QueuedConnection-slot draait dan netjes
    # in de Qt event-thread waar QMessageBox veilig is.
    _crash_reported = pyqtSignal(str, str)
    ...
    def __init__(self):
        ...
        # werk wegblaast. Manager start zo dat ook de initiële (lege)
        # workspace al wordt bewaakt.
        self.autosave = AutosaveManager(self)
        self.autosave.attach_workspace(self.ws)
        self.autosave.start()

        # Crash-handler dialog-callback. crash.install_handlers() is
        # (idealiter) in __main__.py al aangeroepen vóór MainWindow
        # bestond — die fase had alleen disk-logging. Nu we wél een
        # window hebben, upgraden we de callback naar één die ook
        # een non-blocking dialog toont. De signal-emit is thread-safe;
        # de slot draait in de Qt event-thread.
        self._crash_reported.connect(
            self._on_crash_reported, Qt.ConnectionType.QueuedConnection
        )
        crash_mod.install_handlers(
            dialog_callback=lambda summary, log_path:
                self._crash_reported.emit(summary, str(log_path))
        )

        # Recovery-prompt voor orphan untitled-autosaves uit eerdere,
        # gecrashte sessies. Even uitstellen tot ná de event-loop draait
        # (anders verschijnt de dialog vóór de hoofdwindow zichtbaar is).
        QTimer.singleShot(400, self._check_orphan_recovery)
    ...
    def _build_menu(self) -> None:
        ...
        m_tr.addSeparator()
        # Workspace-default voor "stop andere audio bij vuren". Per-cue
        # override staat in Inspector → Audio → 'On fire'-dropdown.
        self.act_auto_stop = m_tr.addAction("Auto-stop other audio on fire")
        self.act_auto_stop.setCheckable(True)
        self.act_auto_stop.setChecked(
            getattr(self.ws, "auto_stop_others_on_fire", False),
        )
        self.act_auto_stop.setToolTip(
            "When ON, firing a new audio cue first stops any audio cues "
            "that are still playing. Per-cue override available on the "
            "audio inspector's 'On fire' dropdown."
        )
    ...
    def action_open(self) -> None:
        ...
        if not path:
            return
        load_path = Path(path)
        # Autosave-recovery: is er een sidecar-autosave die nieuwer is
        # dan het bestand op disk? Dat wijst op een eerdere abrupte
        # exit. Vraag de operator of die wil herstellen vóór we de
        # oudere on-disk versie laden.
        recoverable = find_recoverable_for(load_path)
        if recoverable is not None:
            r = QMessageBox.question(
                self, "Recover unsaved changes?",
                "An autosave file newer than this workspace was found "
                "(probably from a previous session that didn't exit "
                "cleanly).\n\nLoad the autosave instead?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if r == QMessageBox.StandardButton.Yes:
                load_path = recoverable.autosave_path
        try:
            ws = Workspace.load(load_path)
        except Exception as e:
            QMessageBox.critical(self, "Cannot Open", str(e))
            return
        # Als we van een autosave herstelden, wijs de workspace alsnog
        # naar het *originele* pad — dan schrijft een volgende Save
        # netjes terug naar het echte bestand.
        if recoverable is not None and load_path == recoverable.autosave_path:
            ws.path = recoverable.original_path
            ws.dirty = True  # er stonden nog niet-bewaarde wijzigingen in
        self.ws = ws
        self.controller.set_workspace(self.ws)
        self.cue_list.workspace = self.ws
        self.inspector.workspace = self.ws
    ...
    def action_save(self) -> None:
        ...
        try:
            ...
            return
        # Sidecar-autosave is nu redundant — de echte workspace is up
        # to date op disk. Opruimen voorkomt dat een latere recovery-
        # prompt onterecht 'unsaved changes found' zegt.
        if hasattr(self, "autosave"):
    ...
    def action_save_as(self) -> None:
        ...
        try:
            ...
            QMessageBox.critical(self, "Cannot Save", str(e))
            return
        # Workspace heeft nu een nieuw pad → opnieuw attachen zodat
        # toekomstige autosaves naast het nieuwe bestand landen, niet
    ...
    def action_go(self) -> None:
        ...
        # Autosave: na elke GO leggen we de huidige cuelist-volgorde
        # vast. Tijdens een show is een GO het meest waardevolle moment
        # om een snapshot te hebben — als het systeem nu zou crashen
        # weten we exact waar de operator was.
        if hasattr(self, "autosave"):
            self.autosave.bump()
    ...
    def _on_showtime_toggled(self, on: bool) -> None:
        """Wissel showtime-modus. Bevriest destructieve UI-acties zonder
        de PlaybackController of een lopende cue te raken — GO en Stop
        All blijven werken, want anders kun je geen show meer rijden.

        Geblokkeerd in showtime: cuelist drag-reorder, inspector-edits,
        Cut/Copy/Paste/Delete, Undo/Redo, en alle push_*-mutaties via
        de gate ``_destructive_blocked``.
        """
        self._showtime = on
        # Cuelist + inspector readonly schakelen — beide widgets lezen
        # ``self._showtime`` via de mainwindow-handle die ze al hebben
        # voor command_sink. We zetten een expliciet attribuut zodat een
        # widget kan kiezen voor visuele feedback (greyed out, geen
        # cursor-grip op rows, etc.).
        if hasattr(self.cue_list, "set_readonly"):
            self.cue_list.set_readonly(on)
        if hasattr(self.inspector, "set_readonly"):
            self.inspector.set_readonly(on)
        # Edit-acties expliciet disablen zodat shortcuts (Ctrl+X, Ctrl+V,
        # Delete, Ctrl+Z) niet meer aanslaan en het menu visueel meegaat.
        # Bij UIT laten we QUndoStack zelf de enabled-state van Undo/Redo
        # bepalen via z'n canUndo()/canRedo()-staat.
        if hasattr(self, "act_undo"):
            self.act_undo.setEnabled(False if on else self.undo_stack.canUndo())
        if hasattr(self, "act_redo"):
            self.act_redo.setEnabled(False if on else self.undo_stack.canRedo())
        # Status-bar-feedback voor de operator.
        if on:
            self.statusBar().showMessage(
                "Showtime lock ON — destructive edits blocked", 0
            )
        else:
    ...
    def _on_crash_reported(self, summary: str, log_path: str) -> None:
        """Toont een non-modale dialog wanneer crash.py een exception
        heeft afgevangen. Doel: operator ziet dat er iets fout ging,
        krijgt een handle naar de log, maar de show kan doorgaan
        zonder dat er een blocking modal de cuelist bevriest."""
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Icon.Warning)
        msg.setWindowTitle("liveFire — Unexpected error")
        msg.setText(
            "An unexpected error was caught. The show is still running, "
            "but consider saving your workspace soon."
        )
        msg.setInformativeText(f"{summary}\n\nLog: {log_path}")
        msg.setStandardButtons(QMessageBox.StandardButton.Ok)
        # Non-modaal: open() ipv exec(), zodat een tweede crash de eerste
        # niet blokkeert en de operator gewoon kan blijven GO'en.
        msg.setWindowModality(Qt.WindowModality.NonModal)
        msg.show()

    def _check_orphan_recovery(self) -> None:
        """Bij startup: kijk of er ``untitled-*.livefire.autosave``-
        bestanden uit eerdere (gecrashte) sessies in de app-autosave-
        folder staan. Bied de meest recente aan ter herstel.

        We tonen alleen iets als de huidige workspace nog leeg is —
        anders zou de operator een al gevulde sessie verliezen door
        Yes te klikken in een prompt die hij niet vroeg."""
        if self.ws.cues:
            return
        orphans = find_orphan_untitled()
        if not orphans:
            return
        target = orphans[0]
        # Skip onze eigen huidige autosave-file (mocht die er al zijn).
        own_path = (
            self.autosave._path_resolver(self.ws, self.autosave.session_id)
            if hasattr(self, "autosave") else None
        )
        if own_path is not None and target.autosave_path == own_path:
            if len(orphans) < 2:
                return
            target = orphans[1]
        r = QMessageBox.question(
            ...
            "(probably an unclean shutdown).\n\n"
            f"File: {target.autosave_path.name}\n"
            f"Modified: {self._fmt_mtime(target.autosave_path)}\n\n"
            "Load it now?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        if r != QMessageBox.StandardButton.Yes:
            return
        try:
            ws = Workspace.load(target.autosave_path)
        except Exception as e:
            QMessageBox.critical(self, "Cannot recover", str(e))
            return
        # Geen path zetten — dit was een onbenoemde workspace, dus
        # eerste Save zal Save As triggeren. Wel dirty markeren want
        # de inhoud is niet 'opgeslagen' in de gewone zin.
        ws.path = None
        ws.dirty = True
        self.ws = ws
        self.controller.set_workspace(self.ws)
    ...
    def closeEvent(self, e) -> None:
        ...
        # Autosave-sidecar opruimen — operator heeft net Save gekozen
        # of bewust Discard, dus er is niets meer te recoveren. Stop
        # daarna de timer zodat we niet alsnog tijdens shutdown een
        # disk-write triggeren.
        if hasattr(self, "autosave"):
            self.autosave.clear_for_current()
            self.autosave.stop()
